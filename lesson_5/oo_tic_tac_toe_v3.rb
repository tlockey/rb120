...
class Board
  ...
  def draw(heading=nil)
    system('clear')
    display(heading) unless heading.nil?
    display(get_grid_array)
  end

  def display_available
    grid = get_grid_array(numbered: true)
    heading = ["AVAILABLE SQUARES", " "]
    display(heading + grid)
  end
...
class TTTGame
  ...
  def display_goodbye_message
    ...
    game_over_msg = ['~ * GAME OVER * ~', 'SCORES',
                     "#{player1.name}#{player1_score}",
                     "#{player2.name}#{player2_score}",
                     game_over? ? message_opt.first : message_opt.last,
